...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  const [existing] = await landingModuleService.listSettings({}, { take: 1 })

  const settings = existing
    ? await landingModuleService.updateSettings({ id: existing.id, ...data })
    : await landingModuleService.createSettings(data as any)
